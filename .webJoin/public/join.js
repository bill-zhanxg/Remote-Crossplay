...
if (storedURL) document.getElementById('join-input').value = storedURL;

function action(url) {
	if (!url) return alert('The input field cannot be left blank!');
	document.getElementById('joinContent').classList.add('hidden');
...
function startConnection(url) {
	...
	/** @type {import('socket.io').Socket<import('../../src/types/socket.io').ServerToClientEvents, import('../../src/types/socket.io').ClientToServerEvents, import('../../src/types/socket.io').InterServerEvents, import('../../src/types/socket.io').SocketData>} */
	const socket = io(url);
